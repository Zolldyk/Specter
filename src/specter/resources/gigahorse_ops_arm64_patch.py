...
class TAC_Phi(TAC_Statement):
    ...
    def handle(self, state: SymbolicEVMState):
        ...
        _has_register_method = hasattr(state.registers, 'register')
        if _has_register_method:
            ...
            # ARM64/Rosetta2 compatibility: state.registers is a plain dict, pick first valid arg
            for arg_var in self.arg_vars:
                if arg_var in state.registers and state.registers[arg_var] is not None:
                    most_recent_write_register_name = arg_var
                    break
